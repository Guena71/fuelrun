  ...
  return(
    <><div><LogoBar/>
      <div style={{padding:"20px 16px 80px"}}>
        ...
        {(todaySess||!todayDone)&&(
          <div style={{borderRadius:16,marginBottom:14,overflow:"hidden",background:todayDone?"linear-gradient(135deg,"+GR+"18,"+GR+"08)":"linear-gradient(135deg,"+OR+"18,"+OR+"08)",border:"1.5px solid "+(todayDone?GR:OR)+"44"}}>
            <div style={{padding:"14px 16px",display:"flex",alignItems:"center",gap:12}}>
              <div style={{width:40,height:40,borderRadius:12,background:todayDone?GR+"22":OR+"22",display:"flex",alignItems:"center",justifyContent:"center",fontSize:20,flexShrink:0}}>
                {todayDone?"✅":"🏃"}
              </div>
              ...
              {!todayDone&&(
                <button onClick={validateToday} style={{flexShrink:0,padding:"8px 14px",borderRadius:12,background:OR,border:"none",color:"#fff",fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>
                  Valider ✓
                </button>
              )}
            </div>
        ...
        <div style={{background:SURF,border:"1px solid "+BORD,borderRadius:14,overflow:"hidden",marginBottom:14}}>
          ...
          {!p.stravaProfile?(
            <a href={stravaAuthUrl()} style={{display:"flex",alignItems:"center",gap:14,padding:"14px 16px",borderBottom:"1px solid "+BORD,textDecoration:"none",background:"#FC4C02"}}>
              <div style={{width:36,height:36,borderRadius:10,background:"rgba(255,255,255,0.2)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:20,flexShrink:0}}>🔗</div>
              <div style={{flex:1}}>
                <div style={{fontSize:14,fontWeight:700,color:"#fff"}}>Connecter Strava</div>
                <div style={{fontSize:11,color:"rgba(255,255,255,0.8)",marginTop:2}}>Importer automatiquement tes sorties</div>
              </div>
              <div style={{fontSize:18,color:"rgba(255,255,255,0.8)"}}>›</div>
            </a>
          ):(
            <div style={{display:"flex",alignItems:"center",gap:12,padding:"12px 16px",borderBottom:"1px solid "+BORD,background:GR+"0a"}}>
              <div style={{width:32,height:32,borderRadius:8,background:"#FC4C02",display:"flex",alignItems:"center",justifyContent:"center",fontSize:14,flexShrink:0}}>🔗</div>
              <div style={{flex:1}}>
                <div style={{fontSize:13,fontWeight:600,color:TXT}}>Strava connecté</div>
                <div style={{fontSize:11,color:SUB,marginTop:1}}>{p.stravaProfile.athlete?.firstname||""} {p.stravaProfile.athlete?.lastname||""}</div>
              </div>
              <button onClick={p.onStravaSync} style={{padding:"6px 12px",borderRadius:10,background:GR+"18",border:"1px solid "+GR+"44",color:GR,fontSize:11,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>Sync</button>
            </div>
          )}
          ...
          <div style={{padding:"12px 16px",borderBottom:"1px solid "+BORD}}>
            {planLevel(p.profile)>=1?(
              ...
              <div onClick={function(){setShowGpxUpgrade(true);}} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 14px",borderRadius:10,background:SURF2,border:"1px solid "+BORD,cursor:"pointer"}}>
                ...
                <span style={{fontSize:10,fontWeight:700,color:BL,background:BL+"18",padding:"2px 8px",borderRadius:6}}>Essentiel</span>
              </div>
            )}
          </div>
          {lastTracked&&(
