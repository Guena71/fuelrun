...
import { regInfo } from "../data/races.js";
...
export function CoursesScreen(p){
  ...
  var savedRaces=p.races||[];
  var maxRaces=planLevel(p.profile)>=1?Infinity:1;
  var canAdd=savedRaces.length<maxRaces;
  ...
  function addCustom(){
    if(!n||!d||!dt)return;
    var r={id:Date.now(),name:n,dist:parseFloat(d),type:tp,date:dt,city:"Personnalisé",custom:true};
    if(!canAdd){p.onShowPricing&&p.onShowPricing();return;}
    p.onAddRace(r);
    setShowAdd(false);setN("");setD("");setDt("");
  }

  function handleSelectRace(r){
    var isSaved=savedRaces.some(function(x){return x.id===r.id;});
    if(isSaved){
      p.onSetActiveRace(r.id);
    } else {
      if(!canAdd){p.onShowPricing&&p.onShowPricing();return;}
      p.onAddRace(r);
    }
      ...
      <div style={{padding:"20px 16px 0"}}>
        ...
        {savedRaces.length>0&&(
          <div style={{marginBottom:20}}>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:10}}>
              <div style={{fontSize:13,fontWeight:700,color:TXT}}>Mes objectifs</div>
              {!canAdd&&(
                <div onClick={function(){p.onShowPricing&&p.onShowPricing();}} style={{fontSize:10,fontWeight:700,color:BL,background:BL+"18",border:"1px solid "+BL+"44",borderRadius:12,padding:"3px 10px",cursor:"pointer"}}>
                  Essentiel — illimité
                </div>
              )}
            </div>
            <div style={{display:"flex",flexDirection:"column",gap:8}}>
              {savedRaces.map(function(r){
                var isActive=p.race&&p.race.id===r.id;
                var col=r.type==="trail"?GR:BL;
                var wks=weeksUntil(r.date);
                return(
                  <div key={r.id} style={{display:"flex",alignItems:"center",gap:12,padding:"12px 14px",borderRadius:14,background:isActive?OR+"12":SURF,border:"1.5px solid "+(isActive?OR:BORD)}}>
                    <div onClick={function(){p.onSetActiveRace(r.id);}} style={{flex:1,cursor:"pointer"}}>
                      <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:2}}>
                        {isActive&&<span style={{fontSize:10,fontWeight:700,color:OR,background:OR+"20",borderRadius:6,padding:"1px 6px"}}>ACTIF</span>}
                        <span style={{fontSize:14,fontWeight:600,color:isActive?OR:TXT}}>{r.name}</span>
                      </div>
                      <div style={{fontSize:12,color:SUB}}>{r.city} · <span style={{color:col,fontWeight:600}}>{r.dist} km</span> · {wks > 0 ? wks+" sem." : "Passée"}</div>
                    </div>
                    {!isActive&&(
                      <button onClick={function(){p.onSetActiveRace(r.id);}} style={{fontSize:11,fontWeight:700,color:OR,background:OR+"15",border:"1px solid "+OR+"33",borderRadius:8,padding:"5px 10px",cursor:"pointer",fontFamily:"inherit",whiteSpace:"nowrap"}}>Activer</button>
                    )}
                    <button onClick={function(){p.onRemoveRace(r.id);}} style={{width:28,height:28,borderRadius:"50%",background:SURF2,border:"1px solid "+BORD,color:MUT,fontSize:14,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,fontFamily:"inherit"}}>×</button>
                  </div>
                );
              })}
            </div>
            ...
            <div style={{marginBottom:20}}>
              ...
              <div style={{display:"flex",gap:10,overflowX:"auto",paddingBottom:4,touchAction:"pan-x"}}>
                {sugg.map(function(r){
                  ...
                  var wks=weeksUntil(r.date);
                  return(
                    <div key={r.id} onClick={function(){handleSelectRace(r);}} style={{flexShrink:0,width:160,borderRadius:16,background:isActive?col+"18":SURF,border:"1.5px solid "+(isActive?col:BORD),padding:"14px",cursor:"pointer",position:"relative",overflow:"hidden"}}>
                      ...
                      {(function(){var ri=regInfo(r);return(<a href={ri.url} target="_blank" rel="noopener noreferrer" onClick={function(e){e.stopPropagation();}} style={{display:"block",textAlign:"center",fontSize:10,fontWeight:700,color:col,background:col+"18",border:"1px solid "+col+"33",borderRadius:6,padding:"4px 6px",textDecoration:"none"}}>S'inscrire →</a>);})()}
                      {isSaved&&<div style={{position:"absolute",bottom:10,right:10,width:18,height:18,borderRadius:"50%",background:isActive?col:OR,display:"flex",alignItems:"center",justifyContent:"center",fontSize:10,color:"#fff",fontWeight:700}}>{isActive?"★":"✓"}</div>}
                    </div>
                  );
                })}
              </div>
        ...
        {showAdd?(
          <Card style={{padding:16,marginBottom:14}}>
            <div style={{fontSize:14,fontWeight:600,color:TXT,marginBottom:12}}>Ma course personnalisée</div>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:8,paddingBottom:24}}>
          {sorted.map(function(r){
            ...
            return(
              <Card key={r.id} onClick={function(){handleSelectRace(r);}} style={{cursor:"pointer",borderColor:isActive?col:isSaved?OR+"66":BORD,background:isActive?col+"0f":isSaved?OR+"08":SURF}}>
                <div style={{padding:"14px 16px",display:"flex",alignItems:"center",gap:12}}>
                  <div style={{flex:1}}>
                    <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:4}}>
                      <span style={{fontSize:14,fontWeight:600,color:isActive?col:TXT}}>{r.name}</span>
                      {r.star?<span style={{fontSize:11}}>⭐</span>:null}
                      {isSaved&&!isActive&&<span style={{fontSize:9,fontWeight:700,color:OR,background:OR+"18",borderRadius:4,padding:"1px 5px"}}>ENREGISTRÉ</span>}
                    </div>
                    <div style={{display:"flex",gap:6,flexWrap:"wrap",alignItems:"center"}}>
                      ...
                      <span style={{fontSize:12,color:MUT}}>·</span>
                      <span style={{fontSize:12,color:isActive?col:SUB,fontWeight:isActive?600:400}}>{wks} sem.</span>
                  ...
                  <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:6,flexShrink:0}}>
                    {isActive?<div style={{color:col,fontWeight:700,fontSize:16}}>✓</div>:isSaved?<div style={{color:OR,fontSize:13,fontWeight:700}}>★</div>:<div style={{color:MUT,fontSize:18}}>›</div>}
                    {!r.custom&&(function(){var ri=regInfo(r);return(
                      <a href={ri.url} target="_blank" rel="noopener noreferrer" onClick={function(e){e.stopPropagation();}} style={{fontSize:10,fontWeight:700,color:col,background:col+"18",border:"1px solid "+col+"44",borderRadius:6,padding:"3px 8px",textDecoration:"none",whiteSpace:"nowrap"}}>S'inscrire →</a>
                    );})()}
                  </div>
                </div>
              </Card>
