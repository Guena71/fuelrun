...
export function fmtDuration(min,sec){var totalSec=sec!=null&&sec!==""?parseInt(sec):Math.round((parseFloat(min)||0)*60);if(isNaN(totalSec)||totalSec<=0)return "";var h=Math.floor(totalSec/3600),m=Math.floor((totalSec%3600)/60),s=totalSec%60,showSec=sec!=null&&sec!=="";if(h>0)return h+"h "+String(m).padStart(2,"0")+"min"+(showSec&&s>0?" "+s+"s":"");if(m>0)return m+"min"+(showSec&&s>0?" "+s+"s":"");if(showSec&&s>0)return s+"s";return "0min";}
